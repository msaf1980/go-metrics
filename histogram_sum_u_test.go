...
func TestVSumUHistogram_Add(t *testing.T) {
	r := NewRegistry()
	h := GetOrRegisterVSumUHistogram("histogram", r, []int64{1, 2, 5, 8, 20}, nil)
	// 1, 2, 5, 8, 20, inf
	tests := []struct {
		add  int64
		want []uint64
...
func TestVSumUHistogram_SetNames(t *testing.T) {
	h := NewVSumUHistogram([]int64{10, 20, 50, 80, 100}, nil)
...
func BenchmarkVSumUHistogram05(b *testing.B) {
	h := NewVSumUHistogram([]int64{10, 50, 100, 200, 300}, nil)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		h.Add(50)
	}
