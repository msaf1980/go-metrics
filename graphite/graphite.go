...
func (g *Graphite) writeHistogramMetric(name, bucketName, le string, tags string, v uint64, ts int64) (err error) {
	if tags == "" {
		if g.c.Prefix != "" {
			g.buf.WriteString(g.c.Prefix)
			g.buf.WriteRune('.')
...
func (g *Graphite) send(r metrics.Registry) error {
	...
	r.Each(func(name, tags string, tagsMap map[string]string, i interface{}) error {
		...
		case metrics.HistogramInterface:
			...
			for i, bucketName := range metric.Names() {
				if err = g.writeHistogramMetric(name, bucketName, leAliases[i], tags, vals[i], now); err != nil {
					return err
				}
				total += vals[i]
			}
