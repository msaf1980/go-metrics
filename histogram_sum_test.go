...
func TestNewFixedSumHistogram(t *testing.T) {
	tests := []struct {
		startVal           int64
		endVal             int64
		width              int64
	...
	for i, tt := range tests {
		t.Run("#"+strconv.Itoa(i), func(t *testing.T) {
			got := NewFixedSumHistogram(tt.startVal, tt.endVal, tt.width)
			if tt.labelPrefix != "" {
			...
			if !reflect.DeepEqual(got.Weights(), tt.wantWeights) {
				t.Errorf("NewFixedSumHistogram() weights = %+v, want %+v", got.Weights(), tt.wantWeights)
			}
			...
			if len(got.Labels()) != len(got.Values()) {
				t.Errorf("NewFixedSumHistogram() buckets count =%d, want %d", len(got.Labels()), len(got.Values()))
			}
		})
	}
}

func TestFixedSumHistogram_Add(t *testing.T) {
	startVal := int64(10)
	width := int64(10)
	endVal := int64(50)
	r := NewRegistry()
	h := GetOrRegisterFixedSumHistogram("histogram", r, startVal, endVal, width)
	// 10 20 30 40 50 inf
	tests := []struct {
		add  int64
		want []uint64
...
func TestFixedSumHistogram_SetNames(t *testing.T) {
	startVal := int64(10)
	width := int64(10)
	endVal := int64(50)
	...
	if h.NameTotal() != wantNameTotal {
		t.Errorf("NewFixedSumHistogram() total = %q, want %q", h.NameTotal(), wantNameTotal)
	}
...
func TestFixedSumHistogram_Snapshot(t *testing.T) {
	startVal := int64(10)
	width := int64(10)
	endVal := int64(50)
	h := NewFixedSumHistogram(startVal, endVal, width)
	...
	if got.NameTotal() != h.NameTotal() {
		t.Errorf("NewFixedSumHistogram() total = %q, want %q", got.NameTotal(), h.NameTotal())
	}
...
func TestNewVSumHistogram(t *testing.T) {
	tests := []struct {
		weights            []int64
		names              []string
		labelPrefix        string
	...
	for i, tt := range tests {
		t.Run("#"+strconv.Itoa(i), func(t *testing.T) {
			got := NewVSumHistogram(tt.weights, tt.names)
			if tt.labelPrefix != "" {
				got.AddLabelPrefix(tt.labelPrefix)
			}
			if tt.total != "" {
			...
			if !reflect.DeepEqual(got.Weights(), tt.wantWeights) {
				t.Errorf("NewVSumHistogram() weights = %+v, want %+v", got.Weights(), tt.wantWeights)
			}
			if !reflect.DeepEqual(got.WeightsAliases(), tt.wantWeightsAliases) {
				t.Errorf("NewFixedSumHistogram() weightsAliases =\n%q\nwant\n%q", got.WeightsAliases(), tt.wantWeightsAliases)
			}
			...
			if len(got.Labels()) != len(got.Values()) {
				t.Errorf("NewVSumHistogram() buckets count =%d, want %d", len(got.Values()), len(tt.wantLabels))
			}
		})
	}
}

func TestVSumHistogram_Add(t *testing.T) {
	r := NewRegistry()
	h := GetOrRegisterVSumHistogram("histogram", r, []int64{1, 2, 5, 8, 20}, nil)
	// 1, 2, 5, 8, 20, inf
	tests := []struct {
		add  int64
		want []uint64
...
func TestVSumHistogram_Snapshot(t *testing.T) {
	h := NewVSumHistogram([]int64{10, 20, 50, 80, 100}, nil)
	h.Add(19)
	got := h.Snapshot()
	want := []uint64{1, 1, 0, 0, 0, 0}
...
func BenchmarkFixedSumHistogram(b *testing.B) {
	h := NewFixedSumHistogram(10, 100, 10)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		h.Add(50)
	}
...
func BenchmarkFixedSumHistogramH(b *testing.B) {
	h := NewFixedSumHistogram(10, 100, 10)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		h.Add(1000)
...
func BenchmarkFixedSumHistogramParallel(b *testing.B) {
	h := NewFixedSumHistogram(10, 100, 10)
...
func BenchmarkFixedSumHistogramParallelH(b *testing.B) {
	h := NewFixedSumHistogram(10, 100, 10)
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			h.Add(1000)
...
func BenchmarkVSumHistogram05(b *testing.B) {
	h := NewVSumHistogram([]int64{10, 50, 100, 200, 300}, nil)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		h.Add(50)
	}
...
func BenchmarkVSumHistogram05Parallel(b *testing.B) {
	h := NewVSumHistogram([]int64{10, 50, 100, 200, 300}, nil)
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			h.Add(50)
...
func BenchmarkVSumHistogram20(b *testing.B) {
	h := NewVSumHistogram(
...
func BenchmarkVSumHistogram20Parallel(b *testing.B) {
	h := NewVSumHistogram(
		[]int64{10, 50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800},
		nil)
	b.RunParallel(func(pb *testing.PB) {
...
func BenchmarkVSumHistogram20ParallelH(b *testing.B) {
	h := NewVSumHistogram(
		[]int64{10, 50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800},
		nil)
	b.RunParallel(func(pb *testing.PB) {
...
func BenchmarkVSumHistogram100(b *testing.B) {
	h := NewVSumHistogram(
		[]int64{
...
func BenchmarkVSumHistogram100H(b *testing.B) {
	h := NewVSumHistogram(
		[]int64{
			10, 50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800,
			1900, 2000, 2100, 2200, 2300, 2400, 2500, 2600, 2700, 2800, 2900, 3000, 3100, 3200, 3300, 3400, 3500, 3600, 3700, 3800,
...
func BenchmarkVSumHistogram100Parallel(b *testing.B) {
	h := NewVSumHistogram(
		[]int64{
			10, 50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800,
			1900, 2000, 2100, 2200, 2300, 2400, 2500, 2600, 2700, 2800, 2900, 3000, 3100, 3200, 3300, 3400, 3500, 3600, 3700, 3800,
...
func BenchmarkVSumHistogram100ParallelH(b *testing.B) {
	h := NewVSumHistogram(
		[]int64{
			10, 50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800,
			1900, 2000, 2100, 2200, 2300, 2400, 2500, 2600, 2700, 2800, 2900, 3000, 3100, 3200, 3300, 3400, 3500, 3600, 3700, 3800,
...
func BenchmarkFixedSumHistogram_Values(b *testing.B) {
	h := NewFixedSumHistogram(10, 100, 10)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		h.Add(50)
...
func BenchmarkVSumHistogram05_Values(b *testing.B) {
	h := NewVSumHistogram([]int64{10, 50, 100, 200, 300}, nil)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		h.Add(50)
...
func BenchmarkVSumHistogram20_Values(b *testing.B) {
	h := NewVSumHistogram(
		[]int64{10, 50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800},
		nil)
	b.ResetTimer()
...
func BenchmarkVSumHistogram100_Values(b *testing.B) {
	h := NewVSumHistogram(
		[]int64{
			10, 50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800,
			1900, 2000, 2100, 2200, 2300, 2400, 2500, 2600, 2700, 2800, 2900, 3000, 3100, 3200, 3300, 3400, 3500, 3600, 3700, 3800,
